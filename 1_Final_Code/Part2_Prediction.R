#read the train data (Quarter 2, 2003)install.packages("forecast")
...
test_full <- fread("D:/ADS/ads_team_7-ads_midterm_team7-276bcb1fad71/ads_midterm_team7/files/cleaned_historical_data1_Q32005.csv",select=c('CREDIT SCORE', 'FIRST PAYMENT YEAR', 'FIRST PAYMENT MONTH',
                                                                                                                    'MATURITY YEAR', 'MATURITY MONTH',
                                                                                                                    'METROPOLITAN STATISTICAL AREA (MSA) OR METROPOLITAN DIVISION', 
                                                                                                                    'MORTGAGE INSURANCE PERCENTAGE (MI %)', 'NUMBER OF UNITS',
                                                                                                                    'ORIGINAL COMBINED LOAN-TO-VALUE (CLTV)', 'ORIGINAL DEBT-TO-INCOME (DTI) RATIO', 
...
colnames(test_full) <- c('CREDITSCORE','FIRSTPAYMENTYEAR','FIRSTPAYMENTMONTH',
                     'MATURITYYEAR','MATURITYMONTH',
                     'METROPOLITANSTATISTICALAREA.MSA.ORMETROPOLITANDIVISION',
                     'MORTGAGEINSURANCEPERCENTAGE.MI..','NUMBEROFUNITS',
                     'ORIGINALCOMBINEDLOAN.TO.VALUE.CLTV.','ORIGINALDEBT.TO.INCOME.DTI.RATIO',
...
test<-subset(test_full,select=c('CREDITSCORE','FIRSTPAYMENTYEAR','FIRSTPAYMENTMONTH',
                             'MATURITYYEAR','MATURITYMONTH',
                             'METROPOLITANSTATISTICALAREA.MSA.ORMETROPOLITANDIVISION',
                             'MORTGAGEINSURANCEPERCENTAGE.MI..','NUMBEROFUNITS',
                             'ORIGINALCOMBINEDLOAN.TO.VALUE.CLTV.','ORIGINALDEBT.TO.INCOME.DTI.RATIO',
                             'ORIGINALUPB','ORIGINALLOAN.TO.VALUE.LTV',
                             'POSTALCODE','ORIGINALLOANTERM','NUMBEROFBORROWERS','FIRSTTIMEHOMEBUYERFLAGYES',
                             'FIRSTTIMEHOMEBUYERFLAGNO','FIRSTTIMEHOMEBUYERFLAGNA','METROPOLITAN_AREA_FLAG','MORTGAGE_INSURANCE_FLAG','OWNEROCCUPIEDFLAG','INVESTMENTPROPERTYFLAG',
                             'SECONDHOMESPACEFLAG','RETAILCHANNELFLAG','BROKERCHANNELFLAG','CORRESPONDENTCHANNELFLAG',
                             'TP0NOTSPECIFIEDCHANNELFLAG','PREPAYMENTPENALTYMORTGAGE.PPM.FLAGYES',
...
#*********************************************************************************************************************************
#Graphical 
scatter.smooth(x=train2, y=test$ORIGINALINTERESTRATE)
nrow(train)
test_interest = as.data.frame(test_full$ORIGINALINTERESTRATE)
nrow(test_interest)
#*********************************************************************************************************************************
library(forecast)
...
print(RMSE)
#plotting residuals
interest.res = resid(lm.fit)
plot(train$ORIGINALINTERESTRATE, interest.res,ylab="Residuals", xlab="Waiting Time",main="Old Faithful Eruptions") 
abline(0, 0)  
...
x = fit.knn
set.seed(3333)
knn_fit <- train(ORIGINALINTERESTRATE ~., data = train_backward, method = "knn", tuneLength = 10)
