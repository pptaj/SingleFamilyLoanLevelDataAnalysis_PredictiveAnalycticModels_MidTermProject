  ...
  def run(self):
    ...
    for i in range(quarter -1,quarter + 1):
      ...
      if not (os.path.isfile(cleaned_filePath)):
        ...
        # perf = None
        for performance in pd.read_csv(downloads_filePath, sep="|", header = None, chunksize=100000, index_col = False, iterator=True, names = ["LOAN SEQUENCE NUMBER", \
        	...
        	performance = performance[["LOAN SEQUENCE NUMBER", \
                        "MONTHLY REPORTING PERIOD", \
                        "CURRENT ACTUAL UPB", \
                        "CURRENT LOAN DELINQUENCY STATUS",\
                        "LOAN AGE",\
        ...
        # perf.to_csv(cleaned_filePath, sep=',', index = False)
